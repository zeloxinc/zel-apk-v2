...
export function usePos() {
    ...
    async function loadVariants() {
      try {
        let rows = await db.selectAll<LocalVariant>(
          "SELECT * FROM variants ORDER BY variant_name ASC"
        );
        ...
        if (rows.length === 0) {
          ...
          rows = await db.selectAll<LocalVariant>("SELECT * FROM variants");
        }
  ...
  const completeSale = async () => {
    ...
    try {
      ...
      // Fetch active profile or use fallback IDs
      const activeProfile = await db.selectFirst<LocalProfile>(
        "SELECT shop_id, staff_id FROM profiles LIMIT 1"
      );
      const shopId = activeProfile?.shop_id || "shop-1";
      const staffId = activeProfile?.staff_id || "staff-1";
      ...
      // 2. Save items & deduct stock in SQLite
